...
async create(dto: CreateGroupingDto) {
  const exist = await this.groupingRepo.findOne({
    where: { name: dto.name },
  });

  if (exist) {
    throw new ConflictException('Grouping already exists');
  }
  ...
  async findOne(id: string) {
    ...
    grouping.name = dto.name ?? grouping.name;
    ...
    return { message: 'Grouping updated' };
    }
